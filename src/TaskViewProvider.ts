...
export class TaskViewProvider implements vscode.WebviewViewProvider {
    ...
    private getTaskFileUri(): vscode.Uri | undefined {
    ...
    private async loadTasksFromFile(): Promise<string> {
        const uri = this.getTaskFileUri();
        if (!uri) {
            // 如果没有打开文件夹，则降级使用 workspaceState
            return this._context.workspaceState.get<string>('tasklist.tasks', '[]');
        }
        try {
    ...
    private async saveTasksToFile(data: string): Promise<void> {
        const uri = this.getTaskFileUri();
        if (!uri) {
            // 如果没有打开文件夹，则降级使用 workspaceState
